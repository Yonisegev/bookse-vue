...
async function addReview(bookId, review) {
  console.log('adsafd', review)
  let books = await query()
  review._id = utilService.makeId()
  let bookToUpdate = books.find(book => book._id === bookId)
  bookToUpdate.reviews.unshift(review)
  console.log(bookToUpdate)
  return dbService.put(KEY, bookToUpdate)
}

async function removeReview(bookId, reviewId) {
  let books = await query()
  let bookToUpdate = books.find(book => book._id === bookId)
  const reviewIdx = bookToUpdate.reviews.findIndex(
    review => review._id === reviewId
  )
  if (reviewIdx !== -1) {
    bookToUpdate.reviews.splice(reviewIdx, 1)
    dbService.put(KEY, bookToUpdate)
  } else {
...
function addGoogleBook(googleBook) {
  ...
  if (!googleBook.volumeInfo.authors)
    googleBook.volumeInfo.authors = ['No Author']
  if (!googleBook.volumeInfo.categories)
    googleBook.volumeInfo.categories = ['General']
  const formattedGoogleBook = {
    ...
    listPrice: {
      amount: utilService.getRandomInteger(),
      currencyCode: 'USD',
      isOnSale: googleBook.volumeInfo.readingModes.text,
    },
  }
  const savedBook = save(formattedGoogleBook)
  return savedBook
}
      ...
      reviews: [
        {
          ...
          rate: '⭐⭐⭐',
          readAt: '2020',
          text: 'Generic Book, nothing exciting',
          bookId: '',
        },
      ],
      publishedDate: 1999,
